...
def enviar_mensagem_individual(conexao):
    ...
    for i in range(conexao['last'], len(mensagens)):
        ...
        try:
            ...
            print(f"[ERRO] Conexão perdida com {conexao['addr']}")
            break
...
def enviar_mensagem_todos():
    ...
    for conexao in conexoes:
        try:
            conexao['conn'].send(b'')  # Verifica se o soquete ainda está ativo
            conexoes_ativas.append(conexao)
        except (socket.error, BrokenPipeError):
            print(f"[ERRO] Conexão perdida com {conexao['addr']}")

    conexoes = conexoes_ativas  # Atualiza a lista de conexões ativas
